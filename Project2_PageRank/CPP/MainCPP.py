...
def main():
    ...
    #Read in list.csv
    df = pd.read_csv('Data\listTest1.csv', ).drop_duplicates(subset='Link')
    ...
    #Sort matrix
    df_final = pd.DataFrame(df_div2, unique_links.keys()).stack().sort_values(ascending=False).reset_index()
